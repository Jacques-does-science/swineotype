...
from helpers import REF_2_12, SUIS, main_config, resolver_double, stage1_double
from swineotype.stages import family_of, stage1_score

CFG = {"min_pid": 85.0, "min_cov": 0.8, "plurality": 0.6, "delta": 100,
       "ambig_set": {"1", "14", "2", "1/2"}, "keep_debug": False, "tmp_dir": "tmp",
       "require_wzy": 1, "pair_1_14": {"1", "14"}, "pair_2_1_2": {"2", "1/2"}}


def row(allele, bits, contig="c1", pid=100, length=1000, qlen=1000):
    return "\t".join([allele, contig, str(pid), str(length), str(qlen), "0",
                      str(bits), "1", str(length), "1", str(length)])


def stage1(alleles, rows, **over):
...
def test_family_of(serotype, family):
    assert family_of(serotype, CFG) == family
...
def test_near_duplicate_references_in_one_family_are_not_independent_evidence():
    ...
    r = stage1({"wzy_1": ("1", "wzy"), "wzy_14": ("14", "wzy")},
...
def test_marker_classes_within_a_family_do_add():
    ...
    r = stage1({"wzy_2": ("2", "wzy"), "wzx_2": ("2", "wzx")},
...
def test_an_ordinary_within_family_tie_is_still_resolvable():
    ...
    r = stage1({"wzy_2": ("2", "wzy"), "wzy_1_2": ("1/2", "wzy")},
    ...
    assert r["must_stage2_for_pair"] is True
...
def test_top_9_second_2_does_not_select_the_type_2_family():
    ...
    r = stage1({"wzy_9": ("9", "wzy"), "wzy_2": ("2", "wzy")},
               [row("wzy_9", 2000), row("wzy_2", 500)])

    assert r["top"] == "9" and r["second"] == "2"
    assert r["family_top"] == "type:9"
    assert r["family_decisive"] is True
    assert r["must_stage2_for_pair"] is False, "no within-family site to read"


def test_near_tied_competing_families_stay_unresolved():
    r = stage1({"wzy_14": ("14", "wzy"), "wzy_2": ("2", "wzy")},
               [row("wzy_14", 1000), row("wzy_2", 980)])

    assert {f for f, _ in r["family_ranked"]} == {"1_vs_14", "2_vs_1_2"}
...
def test_a_competitor_is_not_hidden_by_two_same_family_front_runners():
    ...
    r = stage1({"wzy_2": ("2", "wzy"), "wzy_1_2": ("1/2", "wzy"), "wzy_9": ("9", "wzy")},
...
def test_a_decisive_singleton_family_is_called_without_stage_2(patched_stages, tmp_path):
    s1 = stage1_double(top="9", second="2", family_top="type:9",
                       family_second="2_vs_1_2", family_types={"type:9": ["9"]})
    main_mod = patched_stages(s1, forbid_stage2=True)
